...
def handshake(n):
    ...
    for i in range(2, n + 1):
        for j in range(i):
            handshakes[i] += handshakes[j] * handshakes[i - j - 1]
...
def kanpsackSW(maxSize, maxWeight, names, sizes, weights, values):
    ...
    numItems = len(names)
    memo = [[(0,-1) for _ in range(maxWeight +1) ] for _ in range(maxSize + 1)]
        ...
        for j in range(1, maxWeight + 1):
            ...
            for k in range(numItems):
                if sizes[k] <= i and weights[k] <= j:
                    temp_value = memo[i - sizes[k]][j - weights[k]][0] + values[k]
                    if temp_value > temp_memo[0]:
                        temp_memo = (temp_value, k)
                memo[i][j] = (temp_memo[0], temp_memo[1])

    i=maxSize
    j=maxWeight
    result = []
    while memo[i][j][1] != -1:
        result.append(names[memo[i][j][1]])
        i -= sizes[memo[i][j][1]]
        j -= weights[memo[i][j][1]]

    # print()
    # print(memo[maxSize][maxWeight])
